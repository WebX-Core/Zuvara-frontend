...
const WhyUsSection = () => {
  ...
  return (
    <section className="relative w-full lg:h-[140vh] overflow-hidden py-4 lg:py-12 lg:-mt-32">
      {/* Background Shape */}
      <div className="absolute inset-0 w-full h-full hidden lg:flex flex-col justify-around">
        ...
        <div className="w-full space-y-4 lg:space-y-0">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 w-full">
            {whyUsLists.map((list) => (
              <div
                key={list.id}
                className="relative flex flex-col gap-4 items-center lg:text-center px-2 lg:px-4 py-2 lg:py-4 lg:not-first:py-6 rounded-lg bg-babyCare lg:bg-white shadow-md lg:mt-24"
              >
                {!isSmallerDevice && (
                ...
                <div className="h-full flex flex-col justify-between lg:justify-start gap-2 lg:pt-12">
                  <p className="text-lg font-semibold">{list.title}</p>
                  <div className="flex">
                    <p className="text-sm w-3/4 lg:w-full h-full">
                      {list.description}
                    </p>
                    {isSmallerDevice && (
                      <div className="w-1/4 lg:w-full h-full">
                        <img
                          src={list.image}
                          alt={list.title}
