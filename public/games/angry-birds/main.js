...
let composite = Matter.Composites.pyramid(500, 200, 10, 6, 0, 0, function(x, y){
    return Matter.Bodies.rectangle(x, y, 20, 20, {
        render: {
            sprite: {
                texture: '/waves/pig.svg'
            }
        }
    })
})
let pigBodies = Matter.Composite.allBodies(composite)
...
Matter.Events.on(engine, 'afterUpdate', function(event){
    ...
    if(!hasWon){
        let knockedDownPigs = pigBodies.filter(function(body){
            return body.position.y >= 360
        })

        if(knockedDownPigs.length === pigBodies.length && pigBodies.length > 0){
            hasWon = true
            if(window.parent){
                window.parent.postMessage({ type: "angry-birds-win" }, window.location.origin)
            }
        }
    }
})
